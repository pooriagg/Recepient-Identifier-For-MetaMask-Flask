  ...
  return function () {
    ...
    79: [function (require, module, exports) {
      ...
      const onTransaction = async ({
        ...
        const balance = (Number(recepientEtherBalance) / 10 ** 18).toFixed(5);
        const recepientState = await window.ethereum.request({
          method: "eth_getCode",
          params: [transaction.to]
        });
        const state = recepientState === "0x" ? "E.O.A - non smart-contract account" : "S.C.A - smart-contract account";
        const options = {
          method: "POST",
          headers: {
        ...
        } = await fetch(alchemyApi, options).then(res => res.json());
        return {
          content: (0, _snapsUi.panel)([(0, _snapsUi.heading)("Relevant data to recepient address"), (0, _snapsUi.text)(`Recepient ether balance => ${balance} ether`), (0, _snapsUi.text)(`Recepient state => ${state}`), (0, _snapsUi.text)(`Total interaction with recepient => ${transfers.length}`)])
        };
      };
